import type { EpisodeProgress, UserStats } from "./types"
...
export class StatsService {
  ...
  static getEpisodeStats(progress: EpisodeProgress) {
    ...
    const averageScore =
      withFeedback > 0
        ? Math.round(
            progress.translations.filter((t) => t.feedback).reduce((sum, t) => sum + (t.feedback?.score || 0), 0) /
              withFeedback,
          )
        : 0
  ...
  static getStreak(allProgress: EpisodeProgress[]): number {
    ...
    allProgress.forEach((progress) => {
      progress.translations.forEach((translation) => {
        const date = new Date(translation.timestamp).toISOString().split("T")[0]
        dates.add(date)
      })
    })
